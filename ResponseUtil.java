...
public class ResponseUtil {
	...
	public static ResponseEntity<Object> generateResponse(ApiRequest apiRequest) {
		...
		if(!apiResponse.isPresent() || StringUtils.isBlank(apiResponse.get().getResponse())) {
	...
	private static void checkLatency(ApiRequest apiRequest)  {
		logger.info("Got API latency " + apiRequest.getLatency());
		
		try {
			Thread.sleep(apiRequest.getLatency());
		} catch (InterruptedException interruptedException) {
			logger.error("InterruptedException occured while trying to apply latency ::");
